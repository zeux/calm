...
static int sample_compare(const void* a, const void* b) {
	struct ProbIndex* a_ = (struct ProbIndex*)a;
	struct ProbIndex* b_ = (struct ProbIndex*)b;
	if (a_->prob > b_->prob)
		return -1;
	if (a_->prob < b_->prob)
		return 1;
...
static int sample_topp(float* probabilities, int n, float topp, struct ProbIndex* probindex, float coin) {
	...
	qsort(probindex, n0, sizeof(struct ProbIndex), sample_compare);
